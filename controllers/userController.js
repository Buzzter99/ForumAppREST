...
router.get("/isAuthenticated", async (req, res) => {
  return req.user
    ? res.status(200).json(new ApiResponse(200, `Hello, ${req.user.email}`))
    : res.status(200).json({ statusCode: 401, message: "Not authenticated" });
});
...
router.post("/update", privateEndpoint, async (req, res) => {
  try {
    ...
    const newTokenInfo = await signToken({
      username,
      email,
      _id: req.user._id,
    });
    res.cookie(constants.COOKIE_NAME, newTokenInfo, {
      httpOnly: true,
      maxAge: 2 * 60 * 60 * 1000,
      sameSite: "strict",
    });
